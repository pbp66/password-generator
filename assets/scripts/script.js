...
var password = {
	...
	createPassword: function () {
		...
		for (var i = 0; i < this.length; i++) {
			newPassword = newPassword + characterList[Math.floor(Math.random() * characterList.length)];
		}
...
function generatePassword(passwordObj) {
	...
	while (!exitProgram) {
		...
		passwordObj.length = parseInt(lengthInput);
		if (isNaN(passwordObj.length) || !passwordObj.validateLength()) {
		...
		for (var i = 0; i < criteria.length; i++) {
			validInput = passwordObj.validateInput(criteria[i]);
